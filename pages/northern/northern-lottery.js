...
const NorthernLottery = (props) => {
  ...
  const checkBetInfo = () => {
    ...
    switch(currentDigitType.value) {
      ...
      case 'lot3':
        ...
        formattedNumbers = selectedUnit.map(index_unit => selectedTen.map(index_ten => selectedHundred.map(index_hundred => `${index_hundred}${index_ten}${index_unit};`)));
        for (const item_1 of formattedNumbers) {
          for (const item_2 of item_1) {
      ...
      case 'lot4':
        if (script !== '') {
          ...
          formattedNumbers = selectedUnit.map(index_unit => selectedTen.map(index_ten => selectedHundred.map(index_hundred => selectedThousand.map(index_thousand => `${index_thousand}${index_hundred}${index_ten}${index_unit};`))));
  ...
  return (
    <Layout gameType="northern" allBetAmount={allBetAmount} clearAllAmount={() => {setAllBetAmount(0)}} clearAll={() => {clearAll()}}>
      <div>
        ...
        {[
          selectedBetTypeIndex === 0 && 
            <Backpack  
              ...
              thousands={thousands}
              clearAll={() => {clearAll()}}
              updateDigits={(digit, index) => {updateDigits(digit, index)}}
              setDigitAll={digit => {setDigitAll(digit)}}
              clearDigitAll={digit => {clearDigitAll(digit)}}
              key="northern"
              />,
