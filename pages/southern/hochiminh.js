...
const SouthernHochiminh = (props) => {
  ...
  const checkBetInfo = () => {
    ...
    let phrase = "";
    let amount = 0;
    if (restrictList.southern_hochiminh) {
  ...
  /** socket process */
  const getNewResult = () => {
    dispatch(getGameLatestResult('southern-hochiminh'))
      .then((res) => {
        if (res) {
          setResult(res);
        }
  ...
  const getNewGame = () => {
    dispatch(getNewGameInfo('southern-hochiminh'))
      .then((res) => {
        setGameInfo(res);
      })
      .catch((err) => {
  ...
  const handleNewGame = useCallback((game) => {
    getNewGame();
    dispatch(getUserInfo(user.userId));
    dispatch(getGameHistoriesForGameType('southern-hochiminh'));
    dispatch(dispatchController => 
      dispatchController({
        type: 'ENABLE_SOUTHERN_HOCHIMINH',
      })  
    );
    getNewResult();
  ...
  const handleRestrict = useCallback(() => {
    dispatch(dispatchController => 
      dispatchController({
        type: 'RESTRICT_SOUTHERN_HOCHIMINH',
      })  
    );
  });
  ...
  useMemo(() => {
    socket.emit("subscribe_timer", 'southern-hochiminh');
  }, [])

  useEffect(() => {
    dispatch(
      setCurrentGameType({
        label: t("game_types.southern.hochiminh"),
        value: "southern-hochiminh",
      })
    );
  }, [t])
  ...
  useEffect(() => {
    ...
    socket.on("RESTRICT_BET_SOUTHERN_HOCHIMINH", handleRestrict);
    socket.on("ENABLE_BET_SOUTHERN_HOCHIMINH", handleEnable);
    return () => {
      socket.removeAllListeners("TIMER");
      socket.removeAllListeners("START_NEW_GAME");
      socket.removeAllListeners("RESTRICT_BET_SOUTHERN_HOCHIMINH");
      socket.removeAllListeners("ENABLE_BET_SOUTHERN_HOCHIMINH");
    }
  }, []);
  return (
    <Layout 
...
export default RequireAuth(SouthernHochiminh);