...
const SaigonVIP = (props) => {
  ...
  /** socket process */
  const getNewResult = () => {
    dispatch(getGameLatestResult('saigon'))
      .then((res) => {
        if (res) {
          setResult(res);
        }
  ...
  const getNewGame = () => {
    dispatch(getNewGameInfo('saigon'))
      .then((res) => {
        setGameInfo(res);
      })
      .catch((err) => {
  ...
  const handleNewGame = useCallback((game) => {
    getNewGame();
    dispatch(getUserInfo(user.userId));
    dispatch(getGameHistoriesForGameType('saigon'));
    getNewResult();
  ...
  useMemo(() => {
    socket.emit("subscribe_timer", 'saigon');
  }, [])

  useEffect(() => {
    dispatch(
      setCurrentGameType({
        label: t("game_types.vip.saigon"),
        value: "saigon",
      })
    );
  }, [t])
...
export default RequireAuth(SaigonVIP);